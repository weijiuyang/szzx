...
from .models import DailyReport, Project, ProjectDeck, ProjectMember, WeeklyReport
...
class MainWindow(QMainWindow):
    def __init__(
        ...
        self.project_mode_buttons: list[QPushButton] = []
    ...
    def _project_tab(self) -> QWidget:
        ...
        title_box.addWidget(_label("按项目看成员、日报、周报和汇报材料。", "muted"))
        header.addLayout(title_box)
        header.addStretch()
        show_page = QPushButton("展示")
        show_page.setObjectName("primaryButton")
        show_page.setCheckable(True)
        show_page.clicked.connect(lambda checked=False: self._select_project_mode(0))
        ...
        self.project_mode_buttons = [show_page, config_page]
        header.addWidget(show_page)
        header.addWidget(config_page)
        refresh = QPushButton("刷新")
        refresh.clicked.connect(self._refresh_project_workspace)
        ...
        self.project_description = _label("项目负责人可以在这里查看所有开发日报、维护项目周报，并归档定期汇报 PPT。", "muted")
        ...
        self.metric_decks = self._metric_card("PPT", "0")
        ...
        self.upload_deck_button = QPushButton("上传汇报 PPT")
    ...
    def _deck_detail_page(self) -> QWidget:
        ...
        self.deck_detail_title = _label("PPT汇报材料", "heroTitle")
        self.deck_detail_meta = _label("选择项目进展流里的 PPT 后，会在这里显示。", "muted")
        hero_layout.addWidget(back)
        hero_layout.addWidget(_label("汇报材料", "eyebrow"))
    ...
    def _select_project_mode(self, index: int) -> None:
        ...
        for button_index, button in enumerate(self.project_mode_buttons):
            button.setChecked(button_index == index)
            button.setObjectName("primaryButton" if button_index == index else "")
    ...
    def _upload_project_deck(self) -> None:
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "选择汇报 PPT",
            "",
            "PowerPoint (*.ppt *.pptx);;All Files (*)",
        )
        if not file_path:
            return
        title = file_path.rsplit("/", 1)[-1]
        self.db.add_project_deck(project.id, title, file_path)
        self._refresh_project_workspace()
    ...
    def _refresh_project_workspace(self) -> None:
        ...
        decks = self.db.list_project_decks(project.id)
        ...
        self._set_metric(self.metric_decks, len(decks))
        ...
        for member in members:
            ...
            if is_manager or member.name == current_name:
                self.daily_member.addItem(f"{member.name} · {member.role}", (member.name, member.role))

        self.product_feed.clear()
        product_items: list[tuple[str, str, str, str, ProjectDeck | None]] = []
        for member in members:
        ...
        for deck in decks:
            product_items.append(
                (
                    ...
                    deck.created_at.strftime("%m-%d %H:%M"),
                    "汇报材料",
                    deck.title,
                    deck,
                )
        ...
        if is_manager:
            for _, time_text, kind, content, deck in sorted(product_items, reverse=True)[:5]:
                self._add_feed_card(self.product_feed, time_text, kind, content, deck)
        else:
            self._add_feed_card(
                self.product_feed,
                "",
        ...
        visible_daily_reports = daily_reports if is_manager else [
            report for report in daily_reports if report.member_name == current_name
        ]
    ...
    def _current_project_member(self, project: Project, members: list[ProjectMember]) -> ProjectMember | None:
        ...
        for member in members:
            if member.name == current_name:
                return member
        if project.owner == current_name:
            return ProjectMember(0, project.id, current_name, "产品经理", project.created_at)
        return None

    def _can_manage_project(self, project: Project, member: ProjectMember | None) -> bool:
        if project.owner == self.db.display_name():
            return True
        if member is None:
            return False
    ...
    def _add_feed_card(
        ...
        if deck is not None:
            ...
            open_button.clicked.connect(lambda checked=False, selected=deck: self._open_deck_file(selected))
            download_button = QPushButton("下载")
            download_button.setObjectName("smallButton")
            download_button.clicked.connect(lambda checked=False, selected=deck: self._download_deck(selected))
    ...
    def _show_deck_detail(self, deck_id: int) -> None:
        deck = self.db.get_project_deck(deck_id)
        if deck is None:
            QMessageBox.warning(self, "文件记录不存在", "这个 PPT 记录已经不存在。")
            self._refresh_project_workspace()
            return
        self.selected_deck_id = deck.id
        self._render_deck_detail(deck)
        self.project_content_stack.setCurrentIndex(1)

    def _render_deck_detail(self, deck: ProjectDeck) -> None:
        source = Path(deck.file_path)
        status = "文件可用" if source.exists() else "原文件找不到"
        self.deck_detail_title.setText(deck.title)
        self.deck_detail_meta.setText(
            f"{deck.created_at.strftime('%Y-%m-%d %H:%M')} · {status}"
        )
    ...
    def _selected_deck(self) -> ProjectDeck | None:
        if self.selected_deck_id is None:
            return None
        return self.db.get_project_deck(self.selected_deck_id)
    ...
    def _open_deck_file(self, deck: ProjectDeck) -> None:
        source = Path(deck.file_path)
        if not source.exists():
            QMessageBox.warning(self, "文件不存在", "原 PPT 文件找不到，可能被移动或删除了。")
            return
    ...
    def _download_deck(self, deck: ProjectDeck) -> None:
        source = Path(deck.file_path)
        if not source.exists():
            QMessageBox.warning(self, "文件不存在", "原 PPT 文件找不到，暂时不能下载。")
            return
        ...
        try:
            ...
            return
        QMessageBox.information(self, "下载完成", f"PPT 已保存到：\n{target}")
    ...
    def _docs_tab(self) -> QWidget:
        ...
        layout.setContentsMargins(42, 38, 42, 38)
        layout.setSpacing(24)
        title = _label("文档库", "sectionTitle")
        body = _label("第一版先保留入口。单机模式下后续可以做本地文档库、标签、全文索引和内网同步。", "muted")
        layout.addWidget(title)
